...
void MPU9250Driver::handleInput() {
	...
	if (moving_ == 1 && stamp_.seconds() < message.header.stamp.sec - 1) {
		moving_ = 0;
	}
	...
	message.angular_velocity.z = mpu9250_->getAngularVelocityZ() * 0.017 * moving_;
...
void MPU9250Driver::readVel(const geometry_msgs::msg::Twist::SharedPtr message) {
...
void MPU9250Driver::calculateOrientation(sensor_msgs::msg::Imu& imu_message)
{
  ...
  yaw = imu_message.angular_velocity.z / 250;
